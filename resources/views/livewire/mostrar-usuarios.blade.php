    ...
    <div class="flex items-center space-x-4 py-5 lg:py-6 relative">
        ...
        <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
            <li class="flex items-center space-x-2">
                ...
                dark:text-accent dark:hover:text-primary"
                   href="{{ route('catalogos.usuarios.create') }}"><i class="fa fa-plus"></i> Crear nuevo usuario
                </a>
            </li>
        </ul>
        <!-- Búsqueda -->
        <div class="flex absolute right-0" x-data="usePopper({ placement: 'bottom-end', offset: 12 })"
            ...
            <div class="relative mr-4 flex h-8">
                <input :placeholder="isShowPopper ? 'Busca por nombre' : 'Buscar ...'"
            ...
            <div class="card mt-4">
                <table class="is-zebra w-full text-left" wire:loading.class="animation-unload">
                    ...
                    <tr>
                        ...
                        <th class="w-40 bg-accent px-4 py-3 font-semibold uppercase text-white dark:bg-navy-800 dark:text-navy-400 lg:px-5">
                            Opciones
                        </th>
                    </tr>
                    </thead>
                    ...
                    @foreach($usuarios as $usuario)
                        <tr class="animation-load" wire:loading.class.remove="animation-load">
                            <td class="px-4 py-3 sm:px-5">{{ $usuario->name }}</td>
                            <td class="px-4 py-3 sm:px-5">{{ $usuario->identity }}</td>
                            <td class="px-4 py-3 sm:px-5">{{ $usuario->email }}</td>
                            <td class="px-4 py-3 sm:px-5">{{ $usuario->getNivel($usuario->nivel) }}</td>
                            <td class="px-4 py-3 sm:px-5">
                                <div class="badge rounded-full  bg-info/10 text-info hover:bg-info/20">
                                    <a href="{{route('catalogos.usuarios.show', $usuario->id)}}"
                                       title="Mostrar detalles"><i
                                            class="fa fa-circle-info"></i></a>
                                </div>
                                <div
                                    class="badge rounded-full  text-warning bg-warning/10 hover:bg-warning/20">
                                    <a href="{{route('catalogos.usuarios.edit', $usuario->id)}}" title="Editar"><i
                                            class="fa fa-edit"></i></a>
                                </div>
                                <div
                                    class="badge rounded-full text-accent bg-accent/10 hover:bg-accent/20">
                                    <a href="{{route('catalogos.usuarios.generate', $usuario->id)}}"
                                       title="Restaurar contraseña"><i
                                            ...
                                            <div
                                                ...
                                                <div class="mt-4">
                                                    ...
                                                    <div class="space-x-3 mt-4">
                                                        <form
                                                            action="{{ route('catalogos.usuarios.destroy', $usuario->id)}}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button
